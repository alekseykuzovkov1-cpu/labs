...
int main()
{
    ...
    // Конструктор по умолчанию - заполняет все координаты нулями
    
    {
        std::cout << "Задание 1.3" << std::endl;
        Rect rect(1, 2, 3, 4);
    }
...

    {
        ...
        Rect rect1(1, 2, 3, 4); //параметризированный конструктор
        Rect rect2 = rect1; //конструктор копирования
     ...
     */
    /* функция-член принадлежит классу (объявляется внутри класса), имеет доступ к приватным членам класса, вызывается для конкретного объекта (объект.метод()),
        Имеет скрытый параметр this — указатель на объект, может быть константной (void method() const). Обычная функция не принадлежит классу (объявляется вне классов),
        не имеет доступа к приватным членам (кроме дружественных), вызывается независимо (функция(аргументы)), не имеет параметра this, 
        не может быть константной в том же смысле
    */
    ...
    {
       Rect r1(10, 50, 20, 80), r2(30, 70, 40, 60);
...

    {
        ...
        std::cout << "  Координаты: (" << rect.get_left() << "," << rect.get_right() << ")-(" << rect.get_top() << "," << rect.get_bottom() << ")" << std::endl;
        ...
        // 1. Изменение ширины
        std::cout << "  До: left=" << rect.get_left() << ", right=" << rect.get_right()  << ", width=" << rect.get_width() << std::endl;
    
        rect.set_width(60);  // Увеличиваем ширину до 60
    
        std::cout << "  После: left=" << rect.get_left() << ", right=" << rect.get_right() 
              << ", width=" << rect.get_width() << std::endl;
    
        // 2. Изменение высоты
        std::cout << "  До: top=" << rect.get_top() << ", bottom=" << rect.get_bottom() << ", height=" << rect.get_height() << std::endl;
    
        rect.set_height(80);  // Увеличиваем высоту до 80
    
        std::cout << "  После: top=" << rect.get_top() << ", bottom=" << rect.get_bottom() << ", height=" << rect.get_height() << std::endl;
    
        // 3. Проверка обновленных данных
        std::cout << "  Новая ширина: " << rect.get_width() << std::endl;
        std::cout << "  Новая высота: " << rect.get_height() << std::endl;
        std::cout << "  Новая площадь: " << rect.get_square() << std::endl;
    }
...

    {
        ...
        std::cout << "Концентрация спирта в первой бочке стала меньше 50% на итерации " << iteration << std::endl;
    }
...

    {
        ...
        std::cout << "Первые 40 чисел Фибоначчи:" << std::endl;
        ...
        for (int n = 2; n < 40; n++) {
            try {
                ...
                // Копируем результат обратно в current для следующей итерации
                // current = result (но мы должны сохранить размер 2x1)
                current = result;
            
                std::cout << "F(" << n << ") = " << static_cast<long long>(current.get(0, 0)) << std::endl;
            
            } catch (const std::exception& e) {
                std::cout << "Ошибка на итерации " << n << ": " << e.what() << std::endl;
                break;
            }
        }
    }
